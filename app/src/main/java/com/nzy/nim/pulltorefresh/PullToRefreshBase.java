...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout
	...
	private OnRefreshListener<T> mRefreshListener;// 下拉刷新和加载更多的监听器

	private LoadingLayout mHeaderLayout;// 下拉刷新的布局

	private LoadingLayout mFooterLayout;// 上拉加载更多的布局

	private int mHeaderHeight;// HeaderView的高度

	private int mFooterHeight;// FooterView的高度

	private boolean mPullRefreshEnabled = true;// 下拉刷新是否可用

	private boolean mPullLoadEnabled = false;// 上拉加载是否可用

	private boolean mScrollLoadEnabled = false;// 判断滑动到底部加载是否可用
	...
	private ILoadingLayout.State mPullDownState = State.NONE;// 下拉的状态

	private State mPullUpState = State.NONE;// 上拉的状态

	T mRefreshableView;// 可以下拉刷新的View

	private SmoothScrollRunnable mSmoothScrollRunnable;// 平滑滚动的Runnable

	private FrameLayout mRefreshableViewWrapper;// 可刷新View的包装布局
	...
	private void init(Context context, AttributeSet attrs) {
		setOrientation(LinearLayout.VERTICAL);// 设置为垂直
	...
	private void refreshLoadingViewsSize() {
		...
		int headerHeight = (mHeaderLayout != null) ? mHeaderLayout
				.getContentSize() : 0;
		int footerHeight = (mFooterLayout != null) ? mFooterLayout
				.getContentSize() : 0;
		// 如果小于0，则重置为0
		if (headerHeight < 0) {
			headerHeight = 0;
		...
		headerHeight = (mHeaderLayout != null) ? mHeaderLayout
				.getMeasuredHeight() : 0;
		footerHeight = (mFooterLayout != null) ? mFooterLayout
				.getMeasuredHeight() : 0;
		if (footerHeight == 0) {
		...
		/*
		 * headerHeight为下拉的高度，那么没有下拉时并不是因为它不存在，而是它在此时的高度为-headerHeight，所以我们看不到，
		 * footerHeight也是相同道理
		 */
		pTop = -headerHeight;
	...
	public final boolean onInterceptTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_MOVE:
			final float deltaY = event.getY() - mLastMotionY;// 移动的距离
			final float absDiff = Math.abs(deltaY);// 去绝对值
			/*
			...
			if (absDiff > mTouchSlop || isPullRefreshing() || isPullLoading()) {
				mLastMotionY = event.getY();// 获取此时的y的坐标
				// 第一个显示出来，Header已经显示或拉下
				if (isPullRefreshEnabled() && isReadyForPullDown()) {
					/*
					 * 1，Math.abs(getScrollY())>0：表示当前滑动的偏移量的绝对值大于0，
					 * 表示当前HeaderView滑出来了或完全 不可见，存在这样一种case，
					 * 当正在刷新时并且RefreshableView已经滑到顶部，向上滑动，那么我们期望的结果是
	...
	public final boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE:
			final float deltaY = ev.getY() - mLastMotionY;// 位移
			mLastMotionY = ev.getY();// 当前坐标值
			if (isPullRefreshEnabled() && isReadyForPullDown()) {
	...
	public void onPullDownRefreshComplete() {
		...
		if (isPullRefreshing()) {
			...
			/**
			 * 回滚动有一个时间，我们在回滚完成后再设置状态为normal，在将LoadingLayout的状态设置为normal之前，
			 * 我们应该禁止截断Touch事件，因为设里有一个post状态，如果有post的Runnable未被执行时，
			 * 用户再一次发起下拉刷新，如果正在刷新时 ，这个Runnable 再次被执行到，那么就会把正在刷新的状态改为正常状态，这就不符合期望
			 */
			postDelayed(new Runnable() {
				@Override
				public void run() {
					setInterceptTouchEventEnabled(true);// 截断事件
	...
	protected void addRefreshableView(Context context, T refreshableView) {
		 ...
		 * 这样做的原因是，如果此时它的height是MATCH_PARENT，那么footer得到的高度就是0，所以，我们先设置高度很小
	...
	final class SmoothScrollRunnable implements Runnable {
		...
		private final Interpolator mInterpolator;// 动画效果
		...
		public void run() {
			...
			if (mStartTime == -1) {
				...
				/**
				 * 我们做所有的计算节省应用运行时间 我们把1000ms设置为临界值
				 */
