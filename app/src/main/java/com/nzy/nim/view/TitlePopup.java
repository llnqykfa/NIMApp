...
public class TitlePopup extends PopupWindow {
            ...
            public void onItemClick(AdapterView<?> arg0, View arg1, int index, long arg3) {
                switch (index){
                    case 0:
                        ...
                        QYApplication.getInstance().setScanResultHandlerSerilize(scanResultHandler);
                        mContext.startActivity(intent);
                        break;
                    case 1:
                        AddNewFriendsActivity.actionIntent(mContext);
                        break;
                    case 2:
                        RingListActivity.actionIntent(mContext);
                        break;
                }
                //点击子类项后，弹窗消失
                dismiss();
    ...
    private MipcaActivityCapture.ScanResultHandlerSerilize scanResultHandler = new MipcaActivityCapture().new ScanResultHandlerSerilize() {
        ...
        public void onResult(String result) {
            if (result.startsWith("QYID")) {
                ...
                FriendsInfoActivity.actionIntent(mContext, s);
            }else if(result.startsWith("QYMeetingId#")){
                //执行签到方法
                String s = result.replace("QYMeetingId#", "");
                requestSingIn(s);
            }else if(result.equals("http://120.27.159.62/services/student/new/makeRegist")){
                ToastUtil.showShort(mContext,"请到相关大学新生入学处入口扫描签到认证");
            }else if(result.startsWith("QYspecMeetingId#")){
                ToastUtil.showShort(mContext,"请到会议相关入口扫描报道");
            }else if(result.equals(URLs.MEETING_HOST+"/API/specMet/saveMetSign.do?")){
                ToastUtil.showShort(mContext,"请到会议相关入口扫描签到");
            }else{
                SacnResultActivity.actionIntent(mContext,result);
            }
        }
    };
    /**
     ...
     */
    private void requestSingIn(String meetingId){
        HashMap<String, String> hashMap = new HashMap<String, String>();
        hashMap.put("meetingId", meetingId);
        HTTPUtils.postWithToken(mContext, URLs.MEETING_MEETINGSIGNIN, hashMap, new VolleyListener() {

            @Override
            public void onErrorResponse(VolleyError volleyError) {
                ToastUtil.show(mContext, R.string.server_is_busy, Gravity.TOP);
            }
